...
class AnomalyDetector:
    ...
    def get_anomaly_summary(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            "top_anomalies": anomalies.nlargest(10, "anomaly_score").to_dict("records")
    ...
    def get_cluster_profiles(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for cluster_id in df["cluster"].unique():
            ...
            profiles[f"Cluster {cluster_id}"] = {
                ...
                "avg_IVI": float(cluster_data["IVI"].mean())
                if "IVI" in df.columns
                else 0,
                "avg_BSI": float(cluster_data["BSI"].mean())
                if "BSI" in df.columns
                else 0,
                "total_updates": int(cluster_data["total_updates"].sum())
...
class DemandForecaster:
    ...
    def forecast_with_prophet(
        self, daily_df: pd.DataFrame, target_col: str = "total_bio_updates"
    ) -> Dict[str, Any]:
    ...
    def _simple_forecast(
        ...
        forecast_dates = pd.date_range(
            start=last_date + pd.Timedelta(days=1), periods=self.forecast_days
        )
        ...
        historical_data["date"] = pd.to_datetime(historical_data["date"]).dt.strftime(
            "%Y-%m-%d"
        )
        ...
        return {
            ...
            "forecast": forecasts,
            "historical": historical_data.rename(
...
class IdentityLifecyclePredictor:
    ...
    def calculate_update_probability(self, pincode_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "identity_velocity_index" in df.columns:
            ...
            ivi_normalized = (
                df["identity_velocity_index"] - df["identity_velocity_index"].min()
            ) / (
                df["identity_velocity_index"].max()
                - df["identity_velocity_index"].min()
                + 1e-6
            )
        else:
            ivi_normalized = 0.5
        ...
        if "biometric_stress_index" in df.columns:
            bsi_normalized = (
                df["biometric_stress_index"] - df["biometric_stress_index"].min()
            ) / (
                df["biometric_stress_index"].max()
                - df["biometric_stress_index"].min()
                + 1e-6
            )
        else:
            bsi_normalized = 0.5
        ...
        df["update_probability"] = (
            0.4 * ivi_normalized + 0.4 * bsi_normalized + 0.2 * youth_normalized
        )
    ...
    def get_high_priority_pincodes(
        self, df: pd.DataFrame, top_n: int = 100
    ) -> pd.DataFrame:
