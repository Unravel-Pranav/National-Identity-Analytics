...
def get_state_analysis(state_name: str) -> str:
    ...
    if isinstance(state_analytics, pd.DataFrame):
        state_data = state_analytics[
            state_analytics["state"].str.lower() == state_name.lower()
        ]
        if state_data.empty:
...
- Monitor {high_stress_state} for any BSI increases""")
    ...
    if anomaly_count > 5:
        ...
        top_anomalies = (
            [a.get("state", "Unknown") for a in anomalies[:3]] if anomalies else []
        )
        recommendations.append(f"""
...
def refresh_data() -> str:
    ...
    try:
        from src.data_pipeline import get_last_load_time, refresh_pipeline
...
def check_data_freshness() -> str:
    ...
    """
    try:
        from src.data_pipeline import check_for_new_data, get_last_load_time
...
def get_district_analysis(state_name: str, district_name: str) -> str:
    ...
    if pincode_data is None or not isinstance(pincode_data, pd.DataFrame):
        return (
            "District-level data not available. Please ensure pincode data is loaded."
        )
    ...
    if district_data.empty:
        ...
        state_districts = df[df["state_lower"] == state_name.lower()][
            "district"
        ].unique()[:10]
    ...
    # Youth data (age 5-17)
    youth_bio = (
        district_data["bio_age_5_17"].sum()
        if "bio_age_5_17" in district_data.columns
        else 0
    )
    youth_demo = (
        district_data["demo_age_5_17"].sum()
        if "demo_age_5_17" in district_data.columns
        else 0
    )
    youth_enrol = (
        district_data["age_5_17"].sum() if "age_5_17" in district_data.columns else 0
    )

    # Child data (age 0-5)
    child_enrol = (
        district_data["age_0_5"].sum() if "age_0_5" in district_data.columns else 0
    )

    # Adult data (age 18+)
    adult_enrol = (
        district_data["age_18_greater"].sum()
        if "age_18_greater" in district_data.columns
        else 0
    )
    adult_bio = (
        district_data["bio_age_17_"].sum()
        if "bio_age_17_" in district_data.columns
        else 0
    )
    adult_demo = (
        district_data["demo_age_17_"].sum()
        if "demo_age_17_" in district_data.columns
        else 0
    )
    ...
    youth_ratio = (
        (youth_bio + youth_demo) / (total_updates + 1) if total_updates > 0 else 0
    )
...
def list_districts_in_state(state_name: str) -> str:
    ...
    district_agg["total_updates"] = (
        district_agg["total_bio_updates"] + district_agg["total_demo_updates"]
    )
    district_agg = district_agg.sort_values("total_updates", ascending=False)

    result = (
        f"📋 **Districts in {state_name.title()}** ({len(district_agg)} districts)\n\n"
    )
...
class AadhaarAnalysisAgents:
    ...
    def __init__(self, nvidia_api_key: str | None = None):
        ...
        if self.api_key and NVIDIA_AVAILABLE:
            ...
            self.llm = ChatNVIDIA(
                model="meta/llama-3.1-8b-instruct", temperature=0.1, max_tokens=512
            )
        ...
        self.agent = create_react_agent(model=self.llm, tools=ALL_TOOLS)
...
**Tone:** Professional, data-driven, actionable. You're helping government officials make critical decisions."""
    ...
    def chat_with_agent(
        self, query: str, context: Dict[str, Any], return_trace: bool = False
    ) -> str | tuple:
        ...
        if self.agent:
            try:
                ...
                print(
                    f"[DEBUG AGENT] Found {len(conversation_history)} messages in history"
                )
                ...
                for i, msg in enumerate(conversation_history):
                    ...
                    print(
                        f"[DEBUG AGENT] History[{i}]: role={role}, content={content[:60]}..."
                    )
                ...
                print(
                    f"[DEBUG AGENT] Total messages to LLM: {len(messages)} (1 system + {len(conversation_history)} history + 1 current)"
                )
                print("[DEBUG AGENT] ========================================\n")
                ...
                error_trace = [
                    {"step": "ERROR", "error": str(e), "fallback": "simple_chat"}
                ]
...
Answer questions about this data. Be specific and helpful."""
    ...
    def _build_report(
        self, context: Dict[str, Any], insights: List[str], recommendations: List[str]
    ) -> str:
